...
import { DnDPCModalContainer, FieldContainer, InventoryContainer, LeftCol, NameContainer, NoteEditor, PCBirthdayPicker, PickBirthdayButton } from './styles';
...
export const DnDPCModalBase: React.FC<IProps> = ({ className = '', isOpen, onClose, onSave, pc }) => {
    ...
    const [birthday, setBirthday] = useState<DnDDate>(pc.birthday?.date || null);
    ...
    const onPCCreateClick = async () => {
        try {
            const newPC = await dnd.campaign.createPC(
                name,
    ...
    const onBirthdayChange = useCallback((date: DnDDate) => {
        setBirthday(date);
    ...
    const renderBirthdayPicker = () => {
        const anchor = (
            <PickBirthdayButton
                buttonType={ ButtonType.LightReverse }
                className='pick-birthday-btn'
                onClick={ () => setBirthdayIsOpen(!birthdayIsOpen) }
            >
                { birthday?.stringify() || 'pick a birthday'}
            </PickBirthdayButton>
        );
        ...
        return (
            <TinyPopover
                ...
                anchor={ anchor }
                isOpen={ birthdayIsOpen }
                onRequestClose={ () => setBirthdayIsOpen(false) }
                type={ PopoverType.primaryDark }
                placement={ ['bottom'] }
            >
                <PCBirthdayPicker
                    allowYearEdit={ false }
                    defaultSelectedDay={ birthday?.stringify() ?? dnd.campaign.currentDate.stringify() }
                    onDayClick={ onBirthdayChange }
                />
            </TinyPopover>
        );
    };
                    ...
                    <FieldContainer>
                        <div className='label'>birthday</div>
                        {
                            editMode
                                ...
                                : <div>{ pc?.birthday?.date?.stringify() || 'no birthday' }</div>
                        }
                    </FieldContainer>
                    <FieldContainer>
