...
import { EventModel, IDnDEvent } from "./event";
import { IDnDRace } from "./race";
import { ICampaign } from "./types";

type PrivateFields = '_birthday' |
'_busy' |
'_campaign' |
'_inventory' |
'_leveledUp' |
...
export class PCModel extends BaseModel {
    ...
    private _birthday: EventModel = null;
    ...
    public setPc = (pc: IPCRef) => {
        ...
        if (pc.birthday) this._birthday = new EventModel(pc.birthday);
    }

    public update = async (name: string, race: string, classes: string[], age: number, birthday: DnDDate, exp: number, level: number) => {
        if (!this._busy) {
            this._busy = true;
